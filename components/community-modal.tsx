...
import { X, Search, Users, Globe, Plus } from "lucide-react";
...
function CommunityCard({
  ...
  return (
    <div className="group flex flex-col rounded-2xl border border-border bg-white p-4 transition-all duration-150 hover:border-primary/20 hover:shadow-md">
      <div
        className={cn(
          "flex h-10 w-10 items-center justify-center rounded-xl text-[18px] leading-none transition-transform group-hover:scale-110",
          helper.color
        )}
      >
        {helper.emoji}
      ...
      <div className="mt-3 flex items-center justify-between gap-2">
        ...
        >
          {added ? "Added ✓" : <><Plus className="h-3 w-3" />Use</>}
        </button>
      </div>
    </div>
  );
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 text-[13px] text-muted-foreground">
                  {COMMUNITY_HELPERS.length} templates built and shared by the community
                </p>
              </div>
            </div>
            <button
        ...
        {added.size > 0 && (
          <div className="shrink-0 flex items-center justify-between border-t border-border bg-muted/30 px-7 py-3">
            <p className="text-[13px] text-muted-foreground">
              <span className="font-semibold text-foreground">{added.size}</span>{" "}
              template{added.size !== 1 ? "s" : ""} selected
            </p>
            <button
              onClick={onClose}
