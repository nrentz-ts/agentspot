// ─── V2 FROZEN SNAPSHOT — do not edit ────────────────────────────────────────
// This is the original AgentSpot workflow detail page (Option 2 — Agents + Workflow).
"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";
import {
  ArrowLeft, Check, Loader2, XCircle, Play, ChevronRight, Clock,
} from "lucide-react";
...
interface WorkflowStep { id: string; name: string; subtitle: string; type: StepType; emoji: string; }
...
const TYPE_LABEL: Record<StepType, string> = { trigger: "Trigger", action: "Action", check: "Check", output: "Output" };
...
function deriveSteps(slug: string): WorkflowStep[] { return WORKFLOW_TEMPLATES[slug] ?? DEFAULT_STEPS; }

type RunStatus = "completed" | "running" | "failed";
interface WorkflowRun { id: string; label: string; date: string; duration: string; status: RunStatus; failedStepId?: string; activeStepId?: string; }
...
function StepStatusIcon({ status }: { status: StepStatus }) {
  if (status === "completed") return <div className="flex h-5 w-5 items-center justify-center rounded-full bg-emerald-100"><Check className="h-3 w-3 text-emerald-600" /></div>;
  if (status === "running")   return <Loader2 className="h-5 w-5 animate-spin text-sky-500" />;
...
function StepDetailPanel({ step, status, onClose }: { step: WorkflowStep; status: StepStatus; onClose: () => void }) {
    ...
    <div className="flex h-full flex-col border-l border-border bg-white">
      <div className="shrink-0 border-b border-border px-5 py-4">
        ...
        <div className="mt-3 flex items-center gap-2.5">
          <div className={cn("flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-[18px]", s.bg)}>{step.emoji}</div>
          <div>
            <p className="text-[14px] font-semibold text-foreground">{step.name}</p>
            <span className={cn("mt-0.5 inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold", s.badge)}>{TYPE_LABEL[step.type]}</span>
          </div>
        </div>
        <div className="mt-3">
      ...
      <div className="shrink-0 border-b border-border px-5">
        <div className="flex gap-0">
          {(["input", "output"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)} className={cn("border-b-2 px-4 py-2.5 text-[13px] font-medium capitalize transition-colors", tab === t ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground")}>
      ...
      <div className="flex-1 overflow-y-auto p-5">
        {tab === "input" ? (
          <div className="rounded-xl bg-muted/40 p-3">
            <p className="font-mono text-[12px] leading-relaxed text-muted-foreground">{`{"step_id": "${step.id}", "type": "${step.type}", "name": "${step.name}", "triggered_at": "2026-03-20T14:14:23Z"}`}</p>
          </div>
        ) : status === "pending" ? (
          ...
          <div className="rounded-xl bg-red-50 border border-red-100 p-3">
            ...
            <p className="font-mono text-[12px] leading-relaxed text-red-500">Step timed out after 30s. No response from downstream service.</p>
          </div>
        ) : (
          <div className="rounded-xl bg-muted/40 p-3">
            <p className="font-mono text-[12px] leading-relaxed text-muted-foreground whitespace-pre-line">{MOCK_OUTPUTS[step.id] ?? "Step completed successfully."}</p>
          </div>
        )}
      </div>
    </div>
...
export default function WorkflowDetailV2({ id }: { id: string }) {
  ...
  const selectedStepStatus = selectedStep ? getStepStatus(selectedStep, selectedRun, steps) : null;
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-[#F8F9FC]">
      <header className="flex shrink-0 items-center justify-between border-b border-border bg-white px-6 py-3">
        <button onClick={() => router.back()} className="flex items-center gap-1.5 text-[13px] font-medium text-muted-foreground transition-colors hover:text-foreground">
          <ArrowLeft className="h-4 w-4" /> Back to Workflows
        </button>
        <div className="flex items-center gap-2.5">
          <div className={cn("flex h-7 w-7 items-center justify-center rounded-lg text-[15px]", color)}>{emoji}</div>
          <span className="text-[14px] font-semibold text-foreground">{name}</span>
          <span className={cn("rounded-full border px-2.5 py-0.5 text-[11px] font-semibold", runStatusConf.bg, runStatusConf.text, runStatusConf.border)}>{runStatusConf.label}</span>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex w-[240px] shrink-0 flex-col border-r border-border bg-white">
          <div className="shrink-0 border-b border-border px-4 py-3">
            <div className="flex items-center justify-between">
              ...
              <span className="rounded-full bg-muted px-1.5 py-0.5 text-[11px] font-semibold text-muted-foreground">{runs.length}</span>
            </div>
          </div>
          <div className="flex-1 overflow-y-auto divide-y divide-border">
            {runs.map((run) => {
              ...
              return (
                <button key={run.id} onClick={() => { setSelectedRunId(run.id); setSelectedStepId(null); }}
                  className={cn("flex w-full flex-col items-start px-4 py-3.5 text-left transition-colors hover:bg-muted/40", isSelected && "bg-primary/[0.04] border-l-2 border-l-primary")}
                >
                  <div className="flex w-full items-center justify-between gap-2">
                    <span className={cn("text-[13px] font-semibold", isSelected ? "text-primary" : "text-foreground")}>{run.label}</span>
                    <span className={cn("rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide", conf.bg, conf.text, conf.border)}>
            ...
            <div className="flex flex-col items-center gap-0">
              <div className="flex w-[260px] items-center justify-between rounded-2xl border border-border bg-white px-5 py-3.5 shadow-sm">
                <div className="flex items-center gap-2.5">
                  ...
                  <div><p className="text-[13px] font-semibold text-foreground">Start</p><p className="text-[11px] text-muted-foreground">Workflow starts here</p></div>
                </div>
                <div className="flex h-5 w-5 items-center justify-center rounded-full bg-emerald-100"><Check className="h-3 w-3 text-emerald-600" /></div>
              </div>
                ...
                return (
                  <div key={step.id} className="flex flex-col items-center gap-0">
                    <div className="flex h-10 w-px flex-col items-center">
                      ...
                      <svg width="8" height="8" viewBox="0 0 8 8" className="shrink-0"><path d="M4 0 L4 8 M1 5 L4 8 L7 5" stroke="currentColor" strokeWidth="1.5" fill="none" className="text-border" /></svg>
                    </div>
                    <button onClick={() => setSelectedStepId(isActive ? null : step.id)}
                      className={cn("group flex w-[260px] items-center gap-3 rounded-2xl border p-4 text-left transition-all duration-150 hover:shadow-md",
                        isActive ? "border-primary/40 shadow-sm ring-1 ring-primary/20 " + s.bg : s.border + " " + s.bg + " hover:border-primary/30",
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <span className={cn("mt-1.5 inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold", stepStatus === "failed" ? "bg-red-100 text-red-600" : s.badge)}>{TYPE_LABEL[step.type]}</span>
                      </div>
                      <div className="flex items-center gap-1.5 shrink-0">
                        <StepStatusIcon status={stepStatus} />
                        <ChevronRight className={cn("h-3.5 w-3.5 transition-colors", isActive ? "text-primary" : "text-muted-foreground/30 group-hover:text-muted-foreground")} />
                      </div>
                    </button>
                  </div>
                );
              ...
              <div className="flex w-[260px] items-center justify-between rounded-2xl border border-border bg-white px-5 py-3.5 shadow-sm">
                <div className="flex items-center gap-2.5">
                  <div className="flex h-7 w-7 items-center justify-center rounded-md bg-muted text-[14px]">⏹</div>
                  <div><p className="text-[13px] font-semibold text-foreground">End</p><p className="text-[11px] text-muted-foreground">Workflow completes here</p></div>
                </div>
                {selectedRun.status === "completed" && <div className="flex h-5 w-5 items-center justify-center rounded-full bg-emerald-100"><Check className="h-3 w-3 text-emerald-600" /></div>}
                {selectedRun.status === "failed" && <XCircle className="h-5 w-5 text-red-400" />}
              </div>
            </div>
          </div>
        </div>
        ...
        {/* RIGHT: Step details */}
        <div className={cn("w-[300px] shrink-0 transition-all duration-200", selectedStep ? "opacity-100" : "opacity-0 pointer-events-none")}>
          {selectedStep && selectedStepStatus && <StepDetailPanel step={selectedStep} status={selectedStepStatus} onClose={() => setSelectedStepId(null)} />}
          {!selectedStep && <div className="flex h-full items-center justify-center border-l border-border bg-white"><p className="text-[13px] text-muted-foreground/40">Click a step to inspect</p></div>}
        </div>
      </div>
    </div>
  );
